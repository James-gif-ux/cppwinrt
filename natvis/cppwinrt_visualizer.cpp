...
void LoadMetadata(DkmProcess* process, WCHAR const* processPath, std::string_view const& typeName)
{
    ...
    while (true)
    {
        ...
        if (FindMetadata(process, winmd_path))
        {
            ...
            {
                natvis::db->add_database(path_string, [](TypeDef const& type) { return type.Flags().WindowsRuntime(); });
...
{
    XLANG_ASSERT(typeName.find('<') == std::string_view::npos);
    auto type = natvis::db->find(typeNamespace, typeName);
    if (!type)
    {
...
cppwinrt_visualizer::cppwinrt_visualizer()
{
    try
    {
        ...
        natvis::db.reset(new cache(natvis::db_files, [](TypeDef const& type) { return type.Flags().WindowsRuntime(); }));
    }
    catch (...)
    {
